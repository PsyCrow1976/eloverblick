...
Optional UI label: the compose file already sets `net.unraid.docker.webui` so Unraid can offer a WebUI link on `WEB_PORT` from `.env`.
...
3. Open `http://<unraid-ip>:<WEB_PORT>` (or the container WebUI link). Default `WEB_PORT` is `8080`.
...
The published port and the app listen port both come from `WEB_PORT` in `.env` (default `8080`). If that port is already used on Unraid, change it in `.env` and recreate the stack:

```bash
WEB_PORT=8081
```

Then the UI is `http://<unraid-ip>:8081`. Do not hardcode the port in `docker-compose.yml`.

## 6. Check it

- `http://<unraid-ip>:<WEB_PORT>` shows ElOverblik and the selected address.
...
| Port already allocated | Set another `WEB_PORT` in `.env` and recreate the stack. |
